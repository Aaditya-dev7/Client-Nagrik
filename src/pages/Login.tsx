...
export default function Login() {
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-card border rounded animate-fade-in">
      <h1 className="text-xl font-semibold mb-4">{mode === 'login' ? t('auth.login') : t('auth.register', 'Register')}</h1>
      <form onSubmit={submit} className="space-y-3">
        {mode === 'register' && (
          <div>
            <label className="text-sm">{t('auth.name', 'Name')}</label>
        ...
        <button className="w-full bg-primary text-primary-foreground rounded py-2">{mode === 'login' ? t('auth.login') : t('auth.register', 'Register')}</button>
      </form>
      <div className="text-sm mt-3">
        {mode === 'login' ? (
          <button className="underline" onClick={()=>setMode('register')}>{t('auth.need_account', 'Need an account? Register')}</button>
        ) : (
          <button className="underline" onClick={()=>setMode('login')}>{t('auth.have_account', 'Have an account? Login')}</button>
        )}
      </div>
    </div>
  )
